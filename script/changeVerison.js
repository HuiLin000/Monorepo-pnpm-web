...
function writeVersion(newVersion) {
  ...
  fs.writeFile('./package.json', data, (err) => {
    // 写入失败的话。需要终止整个进程
    if (err) {
      console.error('---------【自动版本号程序】package.json 文件写入失败！' + err.message);
      process.exit(1);
    } else {
      shelljs.exec('git add package.json', (err) => {
        if (err) {
          console.error('---------【自动版本号程序】git add package.json 失败！请手动回退版本号' + err.message);
          process.exit(2);
        } else {
          console.log(`---------【自动版本号程序】版本更新完成[${version}]-->[${newVersion}]---------`);
        }
      });
      console.log('');
      console.log('');
